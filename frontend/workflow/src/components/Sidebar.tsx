...
export default function Sidebar() {
    ...
    useEffect(() => {
        ...
        // Dispatch a custom event so HomePage can react immediately
        window.dispatchEvent(new CustomEvent('github-token-changed', { detail: token }));
    }, [token]);
            ...
            <div className="sidebar-section" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="analyzed-repos-list">
                    {recentJobs.slice(0, 5).map((job) => (
                        <div key={job.id} className="repo-item" onClick={() => window.location.href = `/analysis/${job.id}?view=dashboard`}>
